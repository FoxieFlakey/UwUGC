...
pub fn uffd_try_fix_moved(dest: *mut c_void, moved_reported: &mut usize, moved_intended: usize) {
    assert!(
        moved_intended.is_multiple_of(page_size::get()),
        "Moved intended must be multiple of page size"
    );
    assert!(
        dest.addr().is_multiple_of(page_size::get()),
        "Dest pointer must be multiple of page size"
    );
    ...
    if !moved_reported.is_multiple_of(page_size::get()) {
        eprintln!(
            "WEIRD: UFFD report non multiple of page size is moved ({moved_reported} excess of {}), rounding up",
    ...
    nix::errno::Errno::result(unsafe {
        nix::libc::mincore(check_start, nr_to_check, vector.as_mut_ptr())
    })
    .unwrap();
    ...
    eprintln!(
        "QUIRK FIX: UFFD reported only {} pages moved, but {} pages actually moved, the intended was {} pages",
        reported_pages, actual_moved_pages, intended_pages
    );
}
